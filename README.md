...
- **app.py** — Streamlit web app providing an interactive interface for both case studies
...
6. **DBSCAN** — density-based clustering used as a third comparison algorithm, applied to both case studies
7. **PCA (Principal Component Analysis)** — used to reduce dimensionality and validate clustering results (Case Study 2)
8. **Classification** — Naive Bayes and Decision Tree classifiers trained to predict cluster membership for new customers
9. **Interactive Prediction Tools** — both a command-line tool and a Streamlit web app to classify a new customer into a segment instantly
...
### Individual case study scripts
Each case study folder is self-contained. Navigate into either folder and run:
python explore_data.py          # data exploration
python clustering.py            # clustering + evaluation (K-Means, Hierarchical/PCA, DBSCAN)
python classify_clusters.py     # train classifiers
python predict_new_customer.py  # command-line interactive prediction tool

### Interactive web app
From the project root:
pip install streamlit
streamlit run app.py
This launches a browser-based interface where you can select a case study, enter customer details, and get an instant segment prediction.